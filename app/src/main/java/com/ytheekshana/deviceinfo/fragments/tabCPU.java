...
import com.ytheekshana.deviceinfo.CPUUsage;
...
public class tabCPU extends Fragment {
    ...
    private String cUsage, coreText, idleText;
    private Timer timer;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        try {
            ...
            coreText = getString(R.string.core);
            idleText = getString(R.string.idle);
            timer = new Timer();
                ...
                public void run() {
                    for (int corecount = 0; corecount < Runtime.getRuntime().availableProcessors(); corecount++) {
                        try {
                            ...
                            readerCurFreq.close();
                            String settextcorecores = "\t\t" + coreText + " " + corecount + "       " + (int) currentFreq + " Mhz";
                            int finalCorecount1 = corecount;
                            txtCore[corecount].post(() -> txtCore[finalCorecount1].setText(settextcorecores));

                        } catch (Exception ex) {
                            String settextcorecoresEX = "\t\t" + coreText + " " + corecount + "       " + idleText;
                            int finalCorecount = corecount;
